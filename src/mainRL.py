...
def main(maxGeneration, m, psi, topRatioToRecombine, ratioToMutate, seed, verbose, loadFile, logFile):
    envRL = evaluators.PoleBalancing(render=False, verbose=False)
    psi = [4] + psi + [1]

    trainer = CoSyNE(m=m, psi=psi, evaluator=envRL.evaluator, topRatioToRecombine=topRatioToRecombine, ratioToMutate=ratioToMutate, seed=seed, verbose=verbose, loadFile=loadFile)
    ...
    print("Training has ended with top fitness {}".format(trainer.bestFitnessPerGen[-1]))
    askIfExportNetwork(trainer)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--psi', nargs='+', type=int, help="architecture of the fully connected netwok,\
                                                            as a list of neurone per layer **EXCLUDING INPUT AND OUTPUT LAYERS**. Example: --psi 4", default=[3])
